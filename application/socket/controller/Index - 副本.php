...
/*-------------------------------------------------------------------------------------------*/ 
    ...
    /**
     * [dealDone 符合委托信息的被动单子完成]
     * @param  [array] $data [行为发起者的数据]
     * @param  [array] $v    [符合匹配的委托单数据]
     */
    public function dealDone($data,$v){



        $uid =  $data['type'] == 0?input('session.uid'):$v['uid'];
        $vid =  $data['type'] == 0?$v['uid']:input('session.uid');
        $db = $data['type'] == 0?'s_deal_s':'s_deal_b';
        $type = $data['type'] ==0?'3':'4';
        Db::startTrans();
        try{
            //更新该卖单委托信息的状态
            $data4 = [
